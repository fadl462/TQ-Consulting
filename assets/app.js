...
(() => {
  ...
  if(!reduce && innerWidth > 1100){
    ...
    if(sections.length>2){
      ...
      const pageKey=document.body.className.split(' ')[0]||''; const railSets={ 'solutions-page':['Overview','TQ Architecture','Learning Technology','Outcome Architecture','Start a Conversation'], 'about-page':['Overview','The TQ Story','Four Growth Engines','How We Think','Credibility','Start a Conversation'], 'home-page':['Overview','TQ Architecture','Mental Toughness','Signature Learning','Credibility','TQ Store','Start a Conversation'] }; const railNames=railSets[pageKey]||['Overview','Section 2','Section 3','Section 4','Section 5']; sections.forEach((sec,i)=>{const a=document.createElement('a');a.href=`#section-${i+1}`;a.dataset.index=i;const dot=document.createElement('span');dot.className='rail-dot';const label=document.createElement('span');label.className='rail-label';label.textContent=railNames[i]||`Section ${i+1}`;a.append(dot,label);sec.id=`section-${i+1}`;rail.append(a);});
